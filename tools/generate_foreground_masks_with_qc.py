# -*- coding: utf-8 -*-
"""
Generate foreground masks for durian leaf SSL images using SAM2. Balanced v3 + QC tuned.
...
Recommended command:
python tools/generate_foreground_masks_with_qc_tuned.py \
...
DEFAULT_CONFIG = {
    ...
    # Quality control for pseudo foreground masks.
    # The mask is treated as a weak foreground prior, not pixel-level ground truth.
    "QUALITY": {
        "enable_qc": True,

        # Tuned for complex durian-orchard SSL images.
        # Goal: keep most reasonable pseudo masks, but send risky masks to targeted review.
        "auto_accept_score": 80,
        "review_low_score": 65,

        # Area ratio after mask generation. These thresholds are intentionally tolerant:
        # real orchard images may contain close-up leaves or leaf clusters.
        "small_ratio": 0.030,
        "weak_small_ratio": 0.060,
        "large_ratio": 0.680,
        "weak_large_ratio": 0.580,

        # Edge/component/centroid warnings.
        "edge_touch": 0.350,
        "heavy_edge_touch": 0.500,
        "component_warn_count": 8,
        "component_min_area": 300,
        "off_center_dist": 0.550,

        # Candidate ambiguity is common in multi-leaf orchard scenes,
        # so this is a weak warning only.
        "ambiguous_score_margin": 0.010,

        # Lesion-like color consistency is only a weak warning signal.
        # A high threshold avoids over-triggering on branches, soil, old leaves, and sunlight.
        "lesion_min_area": 1500,
        "lesion_inside_min_ratio": 0.100,

        # Rule-based review overrides.
        # These are used by evaluate_mask_quality() to force optional/low-confidence review
        # when several weak risks appear together.
        "force_offcenter_dist": 0.420,
        "force_lesion_inside_ratio": 0.100,
        "force_large_ratio": 0.580,
        "force_small_ratio": 0.120,
        "force_ambiguous_margin": 0.015,

        # Copy non-auto-accepted samples to review folders.
        "copy_review_files": True,
        "copy_optional_review": True,
    },
}
...
def predict_and_merge_foreground(predictor, image_rgb: np.ndarray, cfg: Dict) -> Tuple[np.ndarray, np.ndarray, List[np.ndarray], List[Dict], List[Dict]]:
    ccfg = cfg["CANDIDATE"]
    ...
    return refined, init_for_preview, boxes, selected, candidates
...
# Quality control for pseudo foreground masks
# =========================================================
def component_count(mask: np.ndarray, min_area: int = 300) -> int:
    """Count connected components larger than min_area."""
    mask = ensure_binary(mask)
    num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(mask, connectivity=8)
    count = 0
    for i in range(1, num_labels):
...
def centroid_distance_to_center(mask: np.ndarray) -> float:
    """Normalized centroid distance from image center. 0=center, close to 1=far off-center."""
    mask = ensure_binary(mask)
    h, w = mask.shape[:2]
    ys, xs = np.where(mask > 0)
    if len(xs) == 0:
    ...
    cx = float(xs.mean())
    cy = float(ys.mean())
    dx = abs(cx - w / 2.0) / (w / 2.0 + 1e-6)
    dy = abs(cy - h / 2.0) / (h / 2.0 + 1e-6)
    return float(np.sqrt(dx * dx + dy * dy) / np.sqrt(2.0))


def lesion_like_mask(image_rgb: np.ndarray) -> np.ndarray:
    """
    Rough brown/yellow/necrotic region detector.

    This is NOT a disease label. It is used only as a quality-control warning:
    if many lesion-like pixels exist outside the foreground mask, the mask may have selected
    the wrong green region.
    """
    hsv = cv2.cvtColor(image_rgb, cv2.COLOR_RGB2HSV)
    h, s, v = cv2.split(hsv)
    ...
    lesion = cv2.morphologyEx(lesion, cv2.MORPH_OPEN, kernel_ellipse(3))
    lesion = cv2.morphologyEx(lesion, cv2.MORPH_CLOSE, kernel_ellipse(5))
    lesion = remove_small_regions(lesion, min_area=20)
    return ensure_binary(lesion)


def lesion_inside_mask_ratio(image_rgb: np.ndarray, mask: np.ndarray) -> Tuple[float, int]:
    lesion = lesion_like_mask(image_rgb)
    mask = ensure_binary(mask)
    lesion_area = int(lesion.sum())
    if lesion_area <= 0:
        return 1.0, 0
    inside = int((lesion & mask).sum())
    return float(inside / max(lesion_area, 1)), lesion_area


def candidate_score_margin(candidates: Optional[List[Dict]]) -> float:
    """Difference between the top two candidate scores. Small margin means ambiguous selection."""
...
def evaluate_mask_quality(
    ...
    mask: np.ndarray,
    selected: Optional[List[Dict]],
    all_candidates: Optional[List[Dict]],
    cfg: Dict,
) -> Dict[str, object]:
    """
    Evaluate generated pseudo mask quality and return status, score, reasons, and metrics.

    Important:
    - quality controls the general confidence level.
    - force_review / force_low are rule-based overrides for typical orchard bad cases.
    - force_review should NOT mean the mask is wrong; it only means it deserves manual inspection.
    """
    qcfg = cfg["QUALITY"]
    mask = ensure_binary(mask)
    h, w = mask.shape[:2]

    ratio = float(mask.sum() / max(h * w, 1))
    edge = edge_touch_ratio(mask)
    comps = component_count(mask, min_area=int(qcfg["component_min_area"]))
    center_dist = centroid_distance_to_center(mask)
    lesion_in_ratio, lesion_area = lesion_inside_mask_ratio(image_rgb, mask)
    margin = candidate_score_margin(all_candidates)
    fill = bbox_fill_ratio(mask)
    aspect = oriented_aspect_ratio(mask)
    contrast = boundary_contrast(image_rgb, mask, ring=5)
    ...
    # These two flags are the key correction.
    # In the previous version, force_review was set but never initialized
    # and never used to override status.
    force_review = False
    force_low = False

    # -----------------------------------------------------
    # Basic geometric quality
    # -----------------------------------------------------
    if ratio < float(qcfg["small_ratio"]):
        quality -= 40
        reasons.append("too_small")
        force_low = True
    elif ratio < float(qcfg["weak_small_ratio"]):
        quality -= 15
        reasons.append("small_mask")

    if ratio > float(qcfg["large_ratio"]):
        quality -= 40
        reasons.append("too_large")
        force_low = True
    elif ratio > float(qcfg["weak_large_ratio"]):
        quality -= 15
        reasons.append("large_mask")
        force_review = True

    if edge > float(qcfg["heavy_edge_touch"]):
        quality -= 30
        reasons.append("heavy_edge_touch")
        force_low = True
    elif edge > float(qcfg["edge_touch"]):
        quality -= 15
        reasons.append("edge_touch")
        force_review = True

    if comps > int(qcfg["component_warn_count"]):
        quality -= 20
        reasons.append("fragmented")
        force_review = True

    if center_dist > float(qcfg["off_center_dist"]):
        quality -= 15
        reasons.append("off_center")
        force_review = True

    if margin < float(qcfg["ambiguous_score_margin"]):
        quality -= 10
        reasons.append("ambiguous_candidate")

    # Lesion-like regions are only a weak warning because branches/soil/dry leaves
    # may also be detected as brown/yellow regions in orchard images.
    lesion_warning = (
        lesion_area >= int(qcfg["lesion_min_area"]) and
        lesion_in_ratio < float(qcfg["lesion_inside_min_ratio"])
    )
    if lesion_warning:
        quality -= 15
        reasons.append("lesion_outside_mask")
    ...
    # -----------------------------------------------------
    # Rule-based review overrides for typical bad cases
    # -----------------------------------------------------
    force_offcenter_dist = float(qcfg.get("force_offcenter_dist", 0.42))
    force_lesion_inside_ratio = float(qcfg.get("force_lesion_inside_ratio", 0.10))
    force_large_ratio = float(qcfg.get("force_large_ratio", 0.58))
    force_small_ratio = float(qcfg.get("force_small_ratio", 0.12))
    force_ambiguous_margin = float(qcfg.get("force_ambiguous_margin", 0.015))
    lesion_min_area = int(qcfg.get("lesion_min_area", 1500))

    # 1) The mask is far from image center, while lesion-like pixels are mostly outside.
    #    This catches cases where the mask selects a side leaf/background instead of the diseased leaf.
    if center_dist > force_offcenter_dist and lesion_area >= lesion_min_area and lesion_in_ratio < force_lesion_inside_ratio:
        force_review = True
        reasons.append("force_review_offcenter_lesion_outside")
        if center_dist > 0.58 and lesion_in_ratio < 0.05:
            force_low = True

    # 2) The mask is large but misses lesion-like regions.
    #    It may have selected a large background leaf or a non-target green region.
    if ratio > force_large_ratio and lesion_area >= lesion_min_area and lesion_in_ratio < 0.12:
        force_review = True
        reasons.append("force_review_large_lesion_outside")

    # 3) The mask is small and far off-center.
    #    This is usually a fragment or a wrong side object.
    if ratio < force_small_ratio and center_dist > 0.50:
        force_review = True
        reasons.append("force_review_small_offcenter")
        if ratio < 0.08:
            force_low = True

    # 4) Candidate selection is highly ambiguous and lesion-like pixels are mostly outside.
    if margin < force_ambiguous_margin and lesion_area >= lesion_min_area and lesion_in_ratio < 0.12:
        force_review = True
        reasons.append("force_review_ambiguous_lesion_outside")

    # 5) The mask is large and heavily touches the image boundary.
    #    This is a common sign of background leakage in orchard scenes.
    if ratio > float(qcfg["weak_large_ratio"]) and edge > float(qcfg["edge_touch"]):
        force_review = True
        reasons.append("force_review_large_edge_touch")
        if edge > float(qcfg["heavy_edge_touch"]):
            force_low = True
    ...
    # -----------------------------------------------------
    if force_low or quality < int(qcfg["review_low_score"]):
        status = "review_low_confidence"
    elif force_review:
        status = "review_optional"
    elif quality >= int(qcfg["auto_accept_score"]):
        status = "auto_accept"
    else:
        status = "review_optional"
    ...
    return {
        "status": status,
        "quality": quality,
        "reasons": "|".join(dedup_reasons) if dedup_reasons else "ok",
        "mask_ratio": ratio,
        "edge_touch": edge,
        "component_count": comps,
        "center_distance": center_dist,
        "lesion_area": int(lesion_area),
        "lesion_inside_ratio": lesion_in_ratio,
        "score_margin": margin,
        "bbox_fill": fill,
        "oriented_aspect": aspect,
        "boundary_contrast": contrast,
        "selected_count": len(selected) if selected is not None else 0,
        "candidate_count": len(all_candidates) if all_candidates is not None else 0,
    }


def ensure_review_dirs(output_root: Path, save_optional: bool = True) -> Dict[str, Path]:
    review_root = output_root / "_review"
    dirs = {
        "review_root": review_root,
        "low_images": review_root / "low_confidence" / "images",
        "low_masks": review_root / "low_confidence" / "masks",
        "low_previews": review_root / "low_confidence" / "previews",
        "optional_images": review_root / "optional" / "images",
        "optional_masks": review_root / "optional" / "masks",
        "optional_previews": review_root / "optional" / "previews",
    }
    for key, d in dirs.items():
        if key == "review_root":
            continue
        if ("optional" in key) and not save_optional:
            continue
...
def copy_to_review(
    ...
    status = str(quality_info["status"])
    if status == "auto_accept":
        return

    if status == "review_low_confidence":
        img_dir = review_dirs["low_images"]
        mask_dir = review_dirs["low_masks"]
        preview_dir = review_dirs["low_previews"]
    else:
        img_dir = review_dirs["optional_images"]
        mask_dir = review_dirs["optional_masks"]
        preview_dir = review_dirs["optional_previews"]
    ...
    try:
        ...
        print(f"[WARN] Failed to copy image to review folder: {img_path} -> {e}")
    save_mask_png(final_mask, mask_dir / f"{img_path.stem}.png")
    if preview is not None:
        Image.fromarray(preview).save(preview_dir / f"{img_path.stem}.jpg")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--no_qc", action="store_true", help="Disable mask quality control and review-copy step.")
    parser.add_argument("--no_copy_review", action="store_true", help="Write quality CSV but do not copy review images/masks/previews.")
    parser.add_argument("--no_optional_review", action="store_true", help="Only copy review_low_confidence, not review_optional.")
...
def main() -> None:
    ...
    cfg["QUALITY"] = DEFAULT_CONFIG["QUALITY"].copy()
    cfg["QUALITY"]["enable_qc"] = not bool(args.no_qc)
    cfg["QUALITY"]["copy_review_files"] = not bool(args.no_copy_review)
    cfg["QUALITY"]["copy_optional_review"] = not bool(args.no_optional_review)
    ...
    quality_csv_path = output_root / "mask_quality_report.csv"
    review_dirs = ensure_review_dirs(output_root, save_optional=cfg["QUALITY"]["copy_optional_review"]) if cfg["QUALITY"]["enable_qc"] else {}
    quality_rows: List[Dict[str, object]] = []

    print("=" * 80)
    print("Foreground SAM2 mask generation - balanced v3 + quality control")
    ...
    print("qc enabled :", cfg["QUALITY"]["enable_qc"])
    print("quality csv:", quality_csv_path)
    print("=" * 80)
    ...
    for img_path in tqdm(image_paths, desc="Generating foreground masks", ncols=140):
        ...
        with torch.inference_mode():
            if use_autocast:
                with torch.autocast("cuda", dtype=torch.bfloat16):
                    final_mask, init_mask, boxes, selected, all_candidates = predict_and_merge_foreground(predictor, image_rgb, cfg)
            else:
                final_mask, init_mask, boxes, selected, all_candidates = predict_and_merge_foreground(predictor, image_rgb, cfg)
        ...
        preview = None
        preview_path = preview_root / f"{img_path.stem}.jpg"
        if cfg["RUNTIME"]["save_preview"]:
            preview = make_preview(image_rgb, init_mask, final_mask, boxes, selected, cfg)
            Image.fromarray(preview).save(preview_path)

        if cfg["QUALITY"]["enable_qc"]:
            quality_info = evaluate_mask_quality(
                image_rgb=image_rgb,
                mask=final_mask,
                selected=selected,
            ...
            row = {
                ...
                "preview_path": str(preview_path) if cfg["RUNTIME"]["save_preview"] else "",
                **quality_info,
            }
            quality_rows.append(row)

            if cfg["QUALITY"]["copy_review_files"] and quality_info["status"] != "auto_accept":
                copy_to_review(
                    img_path=img_path,
    ...
    if cfg["QUALITY"]["enable_qc"]:
        fieldnames = [
            "image_path", "mask_path", "preview_path", "status", "quality", "reasons",
            "mask_ratio", "edge_touch", "component_count", "center_distance",
            "lesion_area", "lesion_inside_ratio", "score_margin", "bbox_fill",
            "oriented_aspect", "boundary_contrast", "selected_count", "candidate_count",
        ]
        with quality_csv_path.open("w", newline="", encoding="utf-8-sig") as f:
        ...
        print("Quality summary:", status_counts)
        print("Quality report saved to:", quality_csv_path)
